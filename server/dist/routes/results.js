...
router.post("/", (req, res) => {
    ...
    try {
        const result = database_1.default
            .prepare(`INSERT INTO race_results (race_id, year, total_time, distance, discipline_data, additional_info, notes)
         VALUES (?, ?, ?, ?, ?, ?, ?)`)
...
router.put("/:id", (req, res) => {
    ...
    if (!existing)
        return res.status(404).json({ error: "Result not found" });
    const { race_id = existing.race_id, year = existing.year, total_time = existing.total_time, distance = existing.distance, discipline_data = JSON.parse(existing.discipline_data), additional_info = JSON.parse(existing.additional_info), notes = existing.notes, } = req.body;
    database_1.default.prepare(`UPDATE race_results
     ...
     WHERE id = ?`).run(race_id, year, total_time, distance, JSON.stringify(discipline_data), JSON.stringify(additional_info), notes, req.params.id);
