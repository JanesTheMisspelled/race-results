...
async function makeThumbnail(data) {
...
router.post("/results/:resultId/images", async (req, res) => {
    const { resultId } = req.params;
    const { filename, mime_type, data, caption } = req.body || {};
    const result = database_1.default.prepare("SELECT id FROM race_results WHERE id = ?").get(resultId);
    if (!result)
    ...
    if (!types_1.ALLOWED_IMAGE_TYPES.includes(mime_type)) {
        return res.status(400).json({ error: `Unsupported image type: ${mime_type}` });
    }
    const count = database_1.default.prepare("SELECT COUNT(*) as c FROM race_images WHERE result_id = ?").get(resultId).c;
    ...
    const sortOrder = database_1.default.prepare("SELECT MAX(sort_order) as m FROM race_images WHERE result_id = ?").get(resultId).m ?? -1;
    try {
        const info = database_1.default
            .prepare(`INSERT INTO race_images (result_id, filename, mime_type, data, thumbnail, caption, sort_order)
         VALUES (?, ?, ?, ?, ?, ?, ?)`)
            .run(resultId, cleanName, mime_type, buf, thumbnail, caption ?? null, sortOrder + 1);
        const row = database_1.default.prepare("SELECT * FROM race_images WHERE id = ?").get(info.lastInsertRowid);
...
router.get("/results/:resultId/images", (req, res) => {
    const { resultId } = req.params;
    const rows = database_1.default
        .prepare("SELECT id, result_id, filename, mime_type, caption, sort_order, created_at, thumbnail FROM race_images WHERE result_id = ? ORDER BY sort_order, id")
        .all(resultId);
    res.json(rows.map(toMeta));
});
// GET /images/:id
router.get("/images/:id", (req, res) => {
    const row = database_1.default.prepare("SELECT mime_type, data FROM race_images WHERE id = ?").get(req.params.id);
    if (!row)
...
router.put("/images/:id", (req, res) => {
    ...
    const existing = database_1.default.prepare("SELECT * FROM race_images WHERE id = ?").get(req.params.id);
    if (!existing)
        return res.status(404).json({ error: "Image not found" });
    ...
    database_1.default.prepare("UPDATE race_images SET caption = ?, sort_order = ? WHERE id = ?").run(newCaption, newOrder, req.params.id);
...
router.delete("/images/:id", (req, res) => {
    const info = database_1.default.prepare("DELETE FROM race_images WHERE id = ?").run(req.params.id);
    if (info.changes === 0)
        return res.status(404).json({ error: "Image not found" });
    res.status(204).send();
});
